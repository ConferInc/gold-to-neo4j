"""Scheduled batch ingestion for recipes."""
...
def main() -> None:
    ...
    try:
        ...
        LOG.info(
            "neo4j_tx_committed",
            extra={"layer": "recipes", "total_rows": total_rows},
        )

        # Best-effort semantic embedding pass (post-commit)
        try:
            from shared.embedding_pass import run_embedding_pass
            emb_summary = run_embedding_pass(config, data, neo4j, layer="recipes")
        ...
        neo4j.close()
        LOG.info("recipes_batch completed")
        ...
        append_run_summary("recipes", summary, ROOT)
