"""Scheduled batch ingestion for customers."""
...
def main() -> None:
    ...
    try:
        ...
        LOG.info(
            "neo4j_tx_committed",
            extra={"layer": "customers", "total_rows": total_rows},
        )

        # Best-effort semantic embedding pass (post-commit)
        try:
            from shared.embedding_pass import run_embedding_pass
            emb_summary = run_embedding_pass(config, data, neo4j, layer="customers")
        ...
        neo4j.close()
        LOG.info("customers_batch completed")
        ...
        append_run_summary("customers", summary, ROOT)
